...
Constructors: `new`, `with_capacity(cap: usize)`, `with_capacity_in<'r>(cap)` (§28.5), `from_array([T; N])`.
Indexing is bounds‑checked in debug builds and MAY be unchecked in release with `#[overflow_checks(false)]`.

**Normative requirement:** `Vec<T>` MUST provide `from_array` for array-to-vector conversion:
```cantrip
function from_array<T, const N: usize>(arr: [T; N]): Vec<T>
    needs alloc.heap;
```
...
### 46.6 Core Functions (Normative)

**Requirement:** Implementations MUST provide the following core functions in `std`:
...
function assert(condition: bool, message: str)
    needs panic;
```
Panics if `condition` is false with the given message.
...
function panic(message: str): !
    needs panic;
```
Immediately terminates execution with the given message. Return type `!` (never) indicates non-returning.

**String Formatting:**
```cantrip
impl String {
    function format(template: str, args: ...): String
        needs alloc.heap;
}
```
Formats a string with positional arguments using `{}` placeholders.

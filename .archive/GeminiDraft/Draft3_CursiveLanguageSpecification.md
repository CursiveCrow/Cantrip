...
**<u>Constraints & Legality</u>**
  ...
  * **Implementation Requirements**: A conforming implementation **MUST**:
    ...
    2.  Reject any program that is ill-formed (violates syntax or static semantics), issuing at least one error diagnostic, unless the violation is classified as **IFNDR** (§1.3.1).
    3.  Support both **Permissive** and **Strict** conformance modes (§1.2).
...
*   **Compilation Unit**: The aggregate of all Normalized Source Files ($S_{norm}$) belonging to a single module, as defined in §5.1.
...
```cursive
// Strict Mode
let admin‮ = false; // Error E-SRC-0308: Bidi override char used to hide variable
```
...
*   Identifiers **MUST NOT** contain any character prohibited by §2.1.3 (e.g., control characters), surrogate code points, or non-characters.
*   **Diagnostic**: Any violation of the identifier grammar (e.g., starting with a digit or emoji) **MUST** trigger diagnostic `E-SRC-0307`.

**2. Reserved Keywords**
*   The sequence of characters constituting an identifier **MUST NOT** match any Reserved Keyword defined in §9.2.1 of Draft 2 (e.g., `procedure`, `let`, `true`).
...
*   However, see §5.3 regarding module path collisions on case-insensitive filesystems.
...
variable_name
_ignored
αβγ           // Greek characters (XID_Start)
...
💩            // Error E-SRC-0307: Emoji are not XID_Start
```
...
**Reserved Namespaces and Patterns** are subsets of syntactically valid identifiers (§2.4) that are semantically restricted by the specification. These identifiers are reserved to ensure future language evolution, ABI stability, and preventing collisions with implementation intrinsics.
...
**2. Vendor Extensions**
*   As defined in §7.3.2, vendor-specific extensions **MUST** use a reverse-domain namespace (e.g., `com.vendor.feature`).
...
**Syntactic Stability** is the property of the language whereby the classification of a sequence of characters into a specific Token Kind (§2.3) is determined solely by the characters themselves and the immediate lexical state (e.g., inside a string literal), without reference to the surrounding syntactic context or parser state.
...
*   The classification of a lexeme as a `<keyword>` (§9.2.1) **MUST** be invariant across all syntactic contexts.
...
*   As defined in §2.3, the lexer **MUST** resolve `>>` as two distinct `>` tokens when parsing generic parameters. This is a specific, bounded exception to the Maximal Munch rule.
...
*   Implementations **MUST NOT** reorder, inject, or suppress tokens during lexical analysis (except for the removal of comments and non-token whitespace as defined in §2.3).
...
2.  **Trailing Operator**: The token immediately **preceding** the newline ($T_{prev}$) is a binary operator (e.g., `+`, `&&`, `=`) or a separator (`,`).
    *   **Set**: All operators defined in §2.3 except postfix operators (if any) and closing delimiters.
...
3.  **Region Storage Duration (Arena)**:
      ...
      * **Allocation**: Upon evaluation of the region allocation operator `^` (§24.4.5).
...
2.  **Permission Subtyping:** A stronger permission is a subtype of a weaker permission (see §4.3).
...
The physical layout of types in memory is classified as follows:
  ...
  * **Defined Behavior (Primitive):** The layout of primitive types is strictly defined by §4.4.
...
The **Unit Type** `()` defined in §4.4.5 is the tuple of arity 0.
...
**Equivalence**
Two tuple types are equivalent if they have the same arity and their corresponding component types are equivalent (§4.1).
...
**Pattern Matching**
Accessing the payload of an enum **MUST** be performed via pattern matching (§8.4). Direct field access is not supported.
...
2.  **Equivalence Collapsing:** If a component type $T_i$ is equivalent to $T_j$ (per §15.2), they are treated as a single component. `i32 | i32` $\rightarrow$ `i32`.
...
**Optimization (Niches)**
Implementations **MUST** apply **Niche Optimization** (see §19.5.1) to Union types where possible.
...
1.  **Direct Access:** A program **MUST NOT** attempt to access the payload of a union directly or reinterpret it as a specific type without checking the tag. Access **MUST** be performed via Pattern Matching (§27) or safe casts.

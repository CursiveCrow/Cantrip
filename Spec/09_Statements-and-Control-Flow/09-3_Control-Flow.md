...
## Clause 9 — Statements and Control Flow

**Part**: IX — Statements and Control Flow
**File**: 09-3_Control-Flow.md  
**Section**: §9.3 Control Flow Statements  
**Stable label**: [stmt.control]  
**Forward references**: §9.1 [stmt.fundamental], §9.2 [stmt.simple], §9.4 [stmt.order], Clause 6 [name], Clause 7 [type], Clause 8 §8.4 [expr.structured]
...
[2] Conditional expressions are specified in Clause 8 §8.4 [expr.structured]. This subsection specifies their behavior when used as statements: result values are discarded, and execution continues sequentially.

##### §9.3.1.2 Syntax Reference

[3] Conditional statement forms reference Clause 8 §8.4:
...
[ Note: Complete syntax and typing rules are in Clause 8 §8.4 [expr.structured]. This subsection specifies statement-specific execution semantics.
— end note ]

##### §9.3.1.3 Constraints

[4] When conditional expressions are used as statements, they shall be well-formed per Clause 8 §8.4. No additional statement-specific constraints apply.
...
When a conditional expression is used as a statement, its value is evaluated per Clause 8 and then discarded:
...
[9] Loop expressions are specified in Clause 8 §8.4.6 [expr.structured]. This subsection specifies their behavior when used as statements.

##### §9.3.2.2 Syntax Reference

[10] Loop statement forms reference Clause 8 §8.4.6:

- **Infinite loop**: `loop block`
- **Conditional loop**: `loop condition block`
- **Iterator loop**: `loop pattern: Type in expr block`

[ Note: Complete syntax and typing rules are in Clause 8 §8.4.6 [expr.structured]. This subsection specifies statement-specific execution semantics.
— end note ]

##### §9.3.2.3 Constraints

[11] When loop expressions are used as statements, they shall be well-formed per Clause 8 §8.4.6.
...
Loops execute per Clause 8 §8.4.6 semantics. When used as statements, result values are discarded:
...
[20] **Context requirement.** Return statements shall appear only within procedure bodies. Return at module scope or in `comptime` blocks produces diagnostic E09-201 (return outside procedure).

[21] **Type compatibility.** When a return value is provided, its type shall match the procedure's declared return type. Type mismatches produce diagnostic E09-202.

[22] **Definite return.** Procedures with non-unit return types shall ensure all control-flow paths produce a return value. Missing returns on some paths trigger diagnostic E09-203 (missing return value).
...
[29] **Context requirement.** Break shall appear within a loop or labeled block. Break outside these contexts produces diagnostic E09-211 (break outside loop or block).

[30] **Label resolution.** When a label is specified, it shall refer to a label declared in an enclosing scope within the same procedure. Undefined labels produce diagnostic E09-212.

[31] **Type unification.** When a loop or labeled block contains multiple `break` statements with values, all values shall have the same type or a common supertype. Type mismatches produce diagnostic E09-213.
...
[37] **Loop context requirement.** Continue shall appear only within loop expressions. Continue outside loops produces diagnostic E09-221.

[38] **Label targets loops.** When labeled, continue shall target a label attached to a loop. Continue targeting a non-loop labeled block produces diagnostic E09-222.
...
- Loop expressions (Clause 8 §8.4.6)
- Block statements
...
[53] **Consistent usage.** A labeled block should use either all valued breaks or all unit breaks. Mixing valued and unit breaks may produce type inference ambiguity (diagnostic E09-213).
...
- Structured expressions (Clause 8)
...
| Code    | Condition                                     |
| ------- | --------------------------------------------- |
| E09-201 | Return statement outside procedure body       |
| E09-202 | Return value type mismatch                    |
| E09-203 | Missing return value on control-flow path     |
| E09-211 | Break statement outside loop or labeled block |
| E09-212 | Break/continue references undefined label     |
| E09-213 | Break values have incompatible types          |
| E09-221 | Continue statement outside loop               |
| E09-222 | Continue targets non-loop label               |
...
1. Support conditional expressions (if, if let, match) as statements per Clause 8 §8.4
2. Support loop expressions (infinite, conditional, iterator) as statements per Clause 8 §8.4.6

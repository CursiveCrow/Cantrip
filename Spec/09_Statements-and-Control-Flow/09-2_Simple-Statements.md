...
**Section**: §9.2 Simple Statements  
**Stable label**: [stmt.simple]  
**Forward references**: §5.2 [decl.variable], §5.7 [decl.initialization], Clause 6 [name], Clause 7 [type], Clause 8 [expr], Clause 11 [memory]
...
#### §9.2.2 Variable Declaration Statements [stmt.simple.vardecl]
...
#### §9.2.3 Assignment Statements [stmt.simple.assign]
...
#### §9.2.5 Empty Statements [stmt.simple.empty]
...
#### §9.2.6 Defer Statements [stmt.simple.defer]
...
[29] The deferred block shall be a block statement with unit type. Value-producing blocks are ill-formed in defer context (diagnostic E09-120).

##### §9.2.6.3 Constraints

[30] **Unit type requirement.** The deferred block shall have type `()`. Blocks producing values trigger diagnostic E09-120.

[31] **No control transfer in defers.** Defer blocks shall not contain `return`, `break`, or `continue` statements referencing outer scopes. Violations produce diagnostic E09-121.
...
```cursive
defer { result 42 }             // error[E09-120]: defer block must have unit type
```
...
| Code    | Condition                                                  |
| ------- | ---------------------------------------------------------- |
| E09-101 | Assignment to immutable (`let`) binding                    |
| E09-102 | Type mismatch in assignment                                |
| E09-120 | Defer block produces non-unit value                        |
| E09-121 | Defer block contains control transfer escaping defer scope |

#### §9.2.8 Conformance Requirements

[38] Implementations shall:

1. Execute variable declarations per §5.2, extending the environment and store
2. Enforce mutability and type constraints for assignments, emitting E09-101 and E09-102

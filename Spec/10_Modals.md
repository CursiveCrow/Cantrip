...
**Part II (Type System):**
- §2.3.3: Modal type overview and basic formation rules
- §2.10: Function types for transition procedures
- §2.8: Tuple types for multiple state transitions
- §2.9: Generic parameters for modal types
- §2.11: Subtyping rules and coercion

CITE: Part II — Type System.

**Part III (Declarations and Scope):**
- §3.1: Module-level declarations
- §3.3: Compile-time evaluation for state invariants
- §3.7: Scope and visibility rules
- §3.9: Name resolution for state names

CITE: Part III — Declarations and Scope.

**Part IV (Lexical Permissions):**
- §4.2: Permission lattice `own ≥ mut ≥ imm`
- §4.5: Move semantics and consumption (modal values are never `Copy`)
- §4.6: Permission coercion rules
- §4.9: Call-boundary permission checking for transition procedures

CITE: Part IV — Lexical Permissions.

**Part V (Expressions and Operators):**
- §5.4: Call expressions for transition procedures
- §5.8: Pattern matching expressions
- §5.2: Evaluation order and determinism
- §5.10: Closure expressions (cannot capture modal values without move)
...
- §6.10: State-aware pattern matching and type refinement
- §6.7: Control flow completeness analysis

CITE: Part VI — Statements and Control Flow.

**Part VII (Contracts and Effects):**
- §7.3: `uses ε` effect clauses on transition procedures
- §7.4: `must P` preconditions for transitions
- §7.5: `will Q` postconditions for transitions
- §7.6: State invariants via `where` clauses
- §7.2: Contract checking at transition boundaries

CITE: Part VII — Contracts and Effects.

**Part VIII (Holes and Inference):**
- §8.1: Hole syntax `_?` extended to states `@_?`
- §8.2: Constraint generation for state inference
- §8.3: Principal solution requirements for state constraints

CITE: Part VIII — Holes and Inference.

**Part IX (Functions and Procedures):**
- §9.2: Function declarations (associated functions in modal types)
- §9.3: Procedure declarations with `self` parameter
- §9.4: Parameter and argument semantics
- §9.6: Call dispatch resolution via `::`

CITE: Part IX — Functions and Procedures.

**Part XIII (Memory, Permissions, and Concurrency):**
- §13.3: Region boundaries and escape analysis for modal values
- §13.7: Thread safety (`Send`/`Sync` for modal types)
- §13.4: Region-polymorphic modal types

CITE: Part XIII — Memory, Permissions, and Concurrency.

**Appendix A (Grammar):**
- Authoritative grammar productions for all modal constructs
...
## 10.14 Complete Examples

### 10.14.1 Example: File Lifecycle (Already Present)

The file lifecycle example is already comprehensive and demonstrates all key features.

### 10.14.2 Example: HTTP Connection Protocol

```cursive
modal HTTPConnection {
    @Idle { host: string, port: u16 }
        where self.port > 0 && self.port < 65536

    @Connecting { host: string, port: u16, socket: Socket, timeout: Duration }

    @Connected { host: string, port: u16, socket: Socket }

    @SendingRequest { host: string, port: u16, socket: Socket, request: HTTPRequest }

    @AwaitingResponse { host: string, port: u16, socket: Socket }

    @ResponseReceived { host: string, port: u16, socket: Socket, response: HTTPResponse }

    @Closed { host: string, port: u16, final_state: ConnectionState }

    @Error { host: string, port: u16, error: NetworkError }

    // Constructor
    function new(host: string, port: u16): HTTPConnection@Idle
        must port > 0 && port < 65536
    {
        result HTTPConnection@Idle { host, port }
    }

    // Transition: Idle -> Connecting
    procedure @Idle -> @Connecting
        connect(self: own HTTPConnection@Idle, timeout: Duration): HTTPConnection@Connecting
        uses net.connect
    {
        let socket = create_socket(self.host, self.port)
        result HTTPConnection@Connecting {
            host: self.host,
            port: self.port,
            socket,
            timeout
    ...
    procedure @Connecting -> @Connected
        complete(self: own HTTPConnection@Connecting): Result<HTTPConnection@Connected, HTTPConnection@Error>
        uses net.poll
    {
        match poll_socket(self.socket, self.timeout) {
            Result@Ok { value: _ } => {
                result Result@Ok { value: HTTPConnection@Connected {
                    host: self.host,
                    port: self.port,
                    socket: self.socket
                } }
            ...
            Result@Err { error } => {
                result Result@Err { error: HTTPConnection@Error {
                    host: self.host,
                    port: self.port,
                    error
                } }
    ...
    // Transition: Connected -> SendingRequest
    procedure @Connected -> @SendingRequest
        send_request(self: own HTTPConnection@Connected, request: HTTPRequest): HTTPConnection@SendingRequest
        uses net.send
    {
        result HTTPConnection@SendingRequest {
            host: self.host,
            port: self.port,
            socket: self.socket,
            request
    ...
    // Transition: SendingRequest -> AwaitingResponse
    procedure @SendingRequest -> @AwaitingResponse
        complete_send(self: own HTTPConnection@SendingRequest): Result<HTTPConnection@AwaitingResponse, HTTPConnection@Error>
        uses net.send
    {
        match send_data(self.socket, self.request.to_bytes()) {
            Result@Ok { value: _ } => {
                result Result@Ok { value: HTTPConnection@AwaitingResponse {
                    host: self.host,
                    port: self.port,
                    socket: self.socket
                } }
    ...
    // Transition: AwaitingResponse -> ResponseReceived | Error
    procedure @AwaitingResponse -> @ResponseReceived
        receive_response(self: own HTTPConnection@AwaitingResponse): Result<HTTPConnection@ResponseReceived, HTTPConnection@Error>
        uses net.recv
    {
        match receive_data(self.socket) {
            Result@Ok { value: data } => {
                let response = parse_http_response(data)
                result Result@Ok { value: HTTPConnection@ResponseReceived {
                    host: self.host,
                    port: self.port,
                    socket: self.socket,
                    response
    ...
    // Transition: ResponseReceived | Connected -> Closed
    procedure @ResponseReceived -> @Closed
        close(self: own HTTPConnection@ResponseReceived): HTTPConnection@Closed
        uses net.close
    {
        close_socket(self.socket)
        result HTTPConnection@Closed {
            host: self.host,
            port: self.port,
            final_state: ConnectionState::CompletedSuccessfully
        }
    }

    procedure @Connected -> @Closed
        close(self: own HTTPConnection@Connected): HTTPConnection@Closed
        uses net.close
    {
        close_socket(self.socket)
        result HTTPConnection@Closed {
            host: self.host,
            port: self.port,
            final_state: ConnectionState::ClosedEarly
        }
    }

    // Error recovery: Error -> Closed
    procedure @Error -> @Closed
        acknowledge(self: own HTTPConnection@Error): HTTPConnection@Closed
    {
        log_error("HTTP connection error: {}", self.error)
        result HTTPConnection@Closed {
            host: self.host,
            port: self.port,
...
// Usage example
procedure fetch_url(url: string): Result<HTTPResponse, Error>
    uses net.connect, net.poll, net.send, net.recv, net.close
{
    let (host, port, path) = parse_url(url)
    let conn = HTTPConnection::new(host, port)

    // Connect
    let connecting = conn::connect(Duration.seconds(10))
    let connected = match connecting::complete() {
        Result@Ok { value: established } => established,
        Result@Err { error } => {
            let closed = error::acknowledge()
            result Result@Err { error: Error.from("Connection failed") }
        }
    }

    // Send request
    let request = HTTPRequest.get(path)
    let sending = connected::send_request(request)
    let awaiting = match sending::complete_send() {
        Result@Ok { value: ready } => ready,
        Result@Err { error } => {
            let closed = error::acknowledge()
            result Result@Err { error: Error.from("Send failed") }
        }
    }

    // Receive response
    let received = match awaiting::receive_response() {
        Result@Ok { value: response_state } => response_state,
        Result@Err { error } => {
            let closed = error::acknowledge()
            result Result@Err { error: Error.from("Receive failed") }
        }
    }

    // Extract response and close
    let response = received.response
    let closed = received::close()

    result Result@Ok { value: response }
}
```

This example demonstrates:
- Complex multi-state protocol (7 states)
- State invariants (`where` clauses)
- Multiple transition paths
- Fallible transitions with error states

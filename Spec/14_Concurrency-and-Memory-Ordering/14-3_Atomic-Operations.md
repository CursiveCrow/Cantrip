...
**Section**: §14.3 Atomic Operations and Orderings  
...
### §14.3 Atomic Operations and Orderings [concurrency.atomic]
...
##### §14.3.3.3 Atomic Compare-Exchange
...
[1] _Atomic type requirement._ Atomic operations shall operate only on primitive integer types and pointer types. Attempting atomic operations on composite types (records, tuples, arrays) produces diagnostic E14-200.

[2] _Permission requirement._ Atomic loads require `const` permission. Atomic stores and RMW operations require `shared` permission. Using atomic operations on `unique` produces diagnostic E14-201.
...
Invalid orderings produce diagnostic E14-202.
...
[5] _Size limits._ Atomic operations are guaranteed for types up to pointer width (32-bit or 64-bit). Larger types (i128, u128) may not support atomic operations on all platforms. Using atomic operations on unsupported types produces diagnostic E14-203.

#### §14.3.5 Semantics

##### §14.3.5.1 Atomicity Guarantee
...
| ------- | ---------------------------------------------- | ---------- |
| E14-200 | Atomic operation on non-atomic type            | [1]        |
| E14-201 | Atomic operation with incorrect permission     | [2]        |
| E14-202 | Invalid memory ordering for operation          | [3]        |
| E14-203 | Atomic operation on unsupported type (e.g. i128) | [5]      |
| E14-204 | Misaligned atomic access                       | [4]        |

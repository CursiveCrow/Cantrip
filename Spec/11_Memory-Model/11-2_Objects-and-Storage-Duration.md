...
**Section**: §11.2 Objects and Storage Duration  
**Stable label**: [memory.object]  
**Forward references**: §11.3 [memory.region], §11.4 [memory.permission], §11.5 [memory.move], Clause 3 §3.1 [basic.object], Clause 3 §3.4 [basic.storage], Clause 5 §5.7 [decl.initialization]
...
[1] This section formalizes the object model introduced conceptually in Clause 3 §3.1. It specifies object properties, storage duration categories, memory locations, and the RAII-based cleanup mechanism that ensures deterministic resource release.
...
#### §11.2.2 Object Model [memory.object.model]

##### §11.2.2.1 Object Definition
...
##### §11.2.2.3 Binding Responsibility

[8] Bindings are classified by cleanup responsibility:

**Responsible bindings** — Created with `=` operator:

```cursive
let x = value       // x is responsible for calling value's destructor
var x = value       // x is responsible for calling destructor
```

**Non-responsible bindings** — Created with `<-` operator:

```cursive
let x <- value      // x is NOT responsible; original binding still responsible
var x <- value      // x is NOT responsible
```

[9] At scope exit, responsible bindings invoke their object's destructor (if any). Non-responsible bindings do not invoke destructors.

[10] Formal responsibility rule:

[ Given: Binding $x$ with cleanup responsibility $r \in \{\text{responsible}, \text{non-responsible}\}$ ]

$$
\frac{\text{binding } x \text{ has responsibility } r = \text{responsible}}{\text{at scope exit: invoke destructor for } x}
\tag{P-Responsibility}
$$

#### §11.2.3 Storage Duration Categories [memory.object.storage]
...
##### §11.2.3.2 Static Storage Duration
...
##### §11.2.3.3 Automatic Storage Duration
...
##### §11.2.3.4 Region Storage Duration
...
##### §11.2.3.5 Heap Storage Duration
...
#### §11.2.4 RAII and Deterministic Cleanup [memory.object.raii]

##### §11.2.4.1 RAII Principle
...
##### §11.2.4.2 LIFO Destruction Order
...
[30] Destructor execution is guaranteed for normal scope exit and early exit via `return`, `break`, or `continue`. Defer statements (§9.2.6) execute before destructors.

##### §11.2.4.4 Non-Responsible Bindings

[31] Non-responsible bindings created with `<-` do not invoke destructors:

```cursive
let responsible = Buffer::new()     // responsible is responsible for cleanup
let viewer <- responsible           // viewer is non-responsible
// At scope exit: responsible calls Buffer destructor
//                viewer does NOT call destructor
```

[32] The original responsible binding retains cleanup responsibility. Multiple non-responsible bindings can reference the same object without creating cleanup obligations.

#### §11.2.5 Destructor Protocol [memory.object.destructor]
...
##### §11.2.5.3 Automatic Field Destruction [memory.object.destructor.fields]
...
##### §11.2.5.4 Destruction Order in Scopes [memory.object.destructor.order]
...
##### §11.2.5.5 Panic and Unwinding [memory.object.destructor.panic]

[41] When a panic occurs during normal execution, destructors execute during stack unwinding (if unwinding is enabled). Destructors for all responsible bindings in scopes being unwound are invoked in LIFO order.

[42] **Panic in destructors.** If a destructor panics during unwinding, the process aborts immediately to prevent double-panic scenarios. Destructors should be designed to be panic-safe; defensive programming practices (catching errors, using `try` blocks) are recommended for cleanup code that might fail.

[43] **Abort-on-panic mode.** Implementations may provide an abort-on-panic mode where panics immediately terminate the process without unwinding. In this mode, destructors are not guaranteed to execute. Programs requiring guaranteed cleanup (files, network connections) should design for graceful shutdown rather than relying on panic unwinding.

##### §11.2.5.6 Types Without Custom Destructors [memory.object.destructor.auto]
...
##### §11.2.5.7 Diagnostics [memory.object.destructor.diagnostics]

| Code    | Condition                                             |
| ------- | ----------------------------------------------------- |
| E11-201 | Drop implementation has incorrect receiver permission |
| E11-202 | Drop implementation missing required grants           |
| E11-203 | Panic in destructor during unwinding                  |

##### §11.2.5.8 Examples [memory.object.destructor.examples]
...
#### §11.2.6 Reference of Value Assignment [memory.object.refvalue]

##### §11.2.6.1 Overview

[46] The `<-` operator creates non-responsible bindings. It establishes a reference to an existing value without assuming cleanup responsibility.
...
[49] Reference of value semantics:

[ Given: Expression $e$ evaluating to value at location $\ell$ ]

$$
\frac{\langle e, \sigma \rangle \Downarrow \langle v, \sigma' \rangle \quad v \text{ at location } \ell}{\langle \texttt{let } x \texttt{ <- } e, \sigma' \rangle \Downarrow \text{Normal}(\sigma'[x \mapsto \ell])}
\tag{E-RefValue}
$$
...
**Example 11.2.6.1 (Reference of value assignment):**
    ...
    [[ |- true => true ]]
{
    ...
    println("Size: {}", viewer.size())

    // At scope exit:
    // - viewer does NOT call Buffer::drop
    // - responsible calls Buffer::drop (cleanup happens once)
}
```

**Example 11.2.6.2 (Multiple non-responsible bindings):**

```cursive
let data = Data::new()              // data is responsible
let ref1 <- data                    // ref1 is non-responsible
let ref2 <- data                    // ref2 is non-responsible
let ref3 <- data                    // ref3 is non-responsible

// All refs access same object
// Only data calls destructor at scope exit
```

#### §11.2.7 Conformance Requirements

[52] Implementations shall:

1. Support all four storage duration categories with specified allocation/deallocation timing
2. Implement RAII: automatically invoke destructors for responsible bindings at scope exit
3. Execute destructors in LIFO order within each scope, with custom `drop` methods called before automatic field destruction
4. Support both responsible (`=`) and non-responsible (`<-`) binding creation
5. Track which bindings have cleanup responsibility via definite assignment analysis
6. Ensure non-responsible bindings do not invoke destructors
7. Provide deterministic cleanup at statically determined program points
8. Maintain object lifetimes per §3.1 and storage duration rules
9. Implement the destructor protocol: call custom `drop` methods when present, then automatically destroy fields in reverse declaration order
10. Handle panic during unwinding per §11.2.5.5, aborting on double-panic scenarios
11. Emit diagnostics E11-201 through E11-203 for destructor violations

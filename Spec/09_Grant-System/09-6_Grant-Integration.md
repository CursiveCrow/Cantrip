...
### 9.6.6.1 Function Restrictions

**Normative Statement 9.6.10**: Functions (pure, side-effect-free) shall not have grant clauses. Functions implicitly have an empty grant set.

**Rationale**: Functions are pure and cannot perform side-effecting operations. Grants authorize side effects.

**Example (informative)**:
```cursive
// Valid: Pure function
function sum(data: [i32; 100]): i32 {
    let total = 0
    for i in 0..100 {
        total += data[i]
    }
...
### 9.6.6.2 Function Calls from Procedures

**Normative Statement 9.6.11**: Procedures may call functions without any grant requirements, as functions require no grants.

**Example (informative)**:
```cursive
function pure_computation(x: i32): i32 {
    result x * x
}
...
5. **Restrict functions** (§9.6.6): Prohibit grant clauses on functions

...
**Section**: §12.7 Sequent Checking Flow
...
### §12.7 Sequent Checking Flow [contract.checking]
...
#### §12.7.2 Call-Site Checking [contract.checking.callsite]
...
##### §12.7.2.2 Grant Checking Algorithm
...
[8] If $G_f \not\subseteq G_g$, the call is ill-formed and produces diagnostic E12-030 listing the missing grants.
...
{
    write_log("Error")  // ❌ E12-030: missing grant io::write
}
```

##### §12.7.2.3 Precondition Checking Algorithm
...
check_precondition_at_call(callee, args, context):
    ...
    match context.verification_mode:
        Static:
            return Error(E12-056, "Cannot prove precondition statically")
...
#### §12.7.3 Procedure Entry Checking [contract.checking.entry]
...
#### §12.7.4 Procedure Exit Checking [contract.checking.exit]
...
check_postcondition_at_exit(callee, result_value, old_captures, exit_state):
    ...
    match callee.verification_mode:
        Static:
            return Error(E12-056, "Cannot prove postcondition statically")
...
#### §12.7.5 Flow-Sensitive Verification [contract.checking.flow]
...
#### §12.7.6 Compositional Checking [contract.checking.compositional]
...
#### §12.7.7 Grant Propagation Through Expressions [contract.checking.propagation]
...
[20] The accumulated grant set for the procedure body must be a subset of the declared grant clause. Missing grants produce diagnostic E12-030.
...
#### §12.7.8 Integration with Definite Assignment [contract.checking.definite]
...
#### §12.7.9 Diagnostics [contract.checking.diagnostics]

[22] Checking flow diagnostics:

| Code    | Condition                                  | Section |
| ------- | ------------------------------------------ | ------- |
| E12-030 | Call site missing required grants          | [8]     |
| E12-056 | Static verification failed (mode=static)   | [9, 13] |
| E12-070 | Grant accumulation exceeds declared grants | [20]    |

#### §12.7.10 Conformance Requirements [contract.checking.requirements]

[23] Implementations shall:

1. Implement call-site grant checking per §12.7.2 algorithm
2. Implement precondition checking per §12.7.2.3 algorithm
3. Implement postcondition checking per §12.7.4 algorithm
...
9. Emit diagnostics E12-030, E12-056, E12-070 for checking failures
10. Provide detailed diagnostic messages showing proof obligations and context

---

**Previous**: §12.6 Invariants: where (§12.6 [contract.invariant]) | **Next**: §12.8 Verification Modes (§12.8 [contract.verification])

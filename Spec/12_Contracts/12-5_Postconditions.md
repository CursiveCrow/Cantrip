...
**Section**: §12.5 Postconditions: will
...
#### §12.5.2 Syntax [contract.postcondition.syntax]
...
#### §12.5.3 The result Identifier [contract.postcondition.result]
...
#### §12.5.4 The @old Temporal Operator [contract.postcondition.old]
...
##### §12.5.4.1 @old Constraints

[13] _Purity requirement._ Expressions within `@old(...)` shall be pure. Effectful expressions produce diagnostic E12-050.

[14] _Parameter scope._ @old expressions may reference parameters and their fields. They shall not reference procedure-local bindings or the `result` identifier. Violations produce diagnostic E12-051.

[15] _No nesting._ `@old` operators shall not nest. `@old(@old(x))` is ill-formed and produces diagnostic E12-009.

[16] _Will clause only._ `@old` is valid only in will clauses. Using it in must clauses produces diagnostic E12-008.

##### §12.5.4.2 @old Semantics
...
#### §12.5.5 Postcondition Verification [contract.postcondition.verification]

##### §12.5.5.1 Static Verification
...
##### §12.5.5.3 Multiple Return Paths
...
#### §12.5.6 Postconditions in Contracts [contract.postcondition.contracts]
...
[26] The implementation's postcondition must imply the contract's postcondition. Implementations may provide stronger guarantees but not weaker ones. Violations produce diagnostic E12-052.
...
#### §12.5.7 Constraints [contract.postcondition.constraints]

[1] _Type requirement._ Predicate expressions in will clauses shall have type `bool`. Non-boolean predicates produce diagnostic E12-053.

[2] _Purity requirement._ Postconditions shall be pure expressions without side effects. Effectful postconditions produce diagnostic E12-054.

[3] _Scope restrictions._ Postconditions may reference parameters, `result`, and `@old` expressions. They shall not reference procedure-local bindings. Violations produce diagnostic E12-055.
...
[5] _@old purity._ Expressions within `@old(...)` shall be pure and reference only parameters or parameter fields. Effectful @old expressions produce diagnostic E12-050; references to locals produce diagnostic E12-051.

[6] _No @old nesting._ The `@old` operator shall not contain nested `@old` operators. Violations produce diagnostic E12-009.
...
#### §12.5.8 Semantics [contract.postcondition.semantics]

##### §12.5.8.1 Guarantee Interpretation
...
##### §12.5.8.2 @old Evaluation Semantics
...
##### §12.5.8.3 Multiple Postconditions
...
#### §12.5.9 Postconditions on All Return Paths [contract.postcondition.paths]
...
#### §12.5.10 Diagnostics [contract.postcondition.diagnostics]

[15] Postcondition diagnostics:

| Code    | Condition                                            | Constraint |
| ------- | ---------------------------------------------------- | ---------- |
| E12-009 | Nested `@old` operators                              | [6]        |
| E12-050 | Effectful expression in `@old(...)`                  | [5]        |
| E12-051 | `@old` references local binding (not parameter)      | [5]        |
| E12-052 | Contract implementation has weaker postcondition     | [26]       |
| E12-053 | Postcondition expression has non-bool type           | [1]        |
| E12-054 | Postcondition performs side effects (not pure)       | [2]        |
| E12-055 | Postcondition references local binding               | [3]        |
| E12-056 | Postcondition unprovable in static verification mode | [20]       |

#### §12.5.11 Conformance Requirements [contract.postcondition.requirements]
...
13. Emit diagnostics E12-009, E12-050 through E12-056 for violations
14. Provide diagnostic messages showing violated postcondition and return point

---

**Previous**: §12.4 Preconditions: must (§12.4 [contract.precondition]) | **Next**: §12.6 Invariants: where (§12.6 [contract.invariant])

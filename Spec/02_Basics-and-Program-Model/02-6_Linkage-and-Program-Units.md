...
public function compute(x: i32): i32 {
...
**No linkage entities**:
- Local variables (all variables declared within functions or blocks)
...
### 2.6.5.2 ODR for Functions

**Requirement 2.6.7 (ODR for Functions)**: A function with external linkage shall have exactly one definition. Multiple declarations are permitted, but only one definition.

**Function definition rule**:

```
[ODR-Function-Single-Definition]
Program P contains function definitions D₁, D₂
Both D₁ and D₂ define function f with same signature
Both D₁ and D₂ have external linkage
────────────────────────────────────────────────────
ERROR E2031: Multiple definitions of function f
(detected at link time)
```

**Declaration vs Definition**:
- **Declaration**: Introduces a name and signature without providing implementation
- **Definition**: Provides the implementation (function body)

**Example (valid - declaration + definition)**:

```cursive
// File: math.cursive
public function sqrt(x: f64): f64  // Declaration (if separate)

public function sqrt(x: f64): f64 {  // Definition
    // Implementation
    result /* ... */
}
```
...
public function compute(x: i32): i32 {  // ❌ Same signature
...
**Inline functions**: Functions marked with the `[[inline]]` attribute may have multiple definitions across compilation units, as the implementation is expected to inline them and perform deduplication.

**Requirement 2.6.8**: The implementation should deduplicate identical inline function definitions. The deduplication mechanism is implementation-defined.

**Generic/monomorphized functions**: Instantiations of generic functions may appear in multiple compilation units. The linker deduplicates identical instantiations.
...
**Undefined behavior**: If multiple definitions of a function or type with external linkage exist and the violation is not detected, the program exhibits undefined behavior (see §[REF_TBD] Undefined Behavior Catalog).
...
**Use cases**:
- Exposing Cursive functions to C code
...
[[extern("C")]]
public function c_ffi_multiply(x: i32, y: i32): i32 {
    result x * y
}
...
[[weak]]
public function default_error_handler(msg: string) {
...
- **E2031**: Multiple definitions of function with external linkage (ODR violation for functions)
- **E2032**: Linkage control attribute applied to non-public entity
- **E2033**: Invalid combination of linkage control attributes
- **E2034**: `[[extern("C")]]` applied to function with non-C-compatible signature
...
5. **ODR enforcement** (§2.6.5): The implementation shall detect and report ODR violations for types and functions with external linkage (errors E2030, E2031).
...
- **Inlining**: Functions with internal linkage or no linkage may be inlined freely

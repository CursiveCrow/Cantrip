...
| **Function scope** | `let x = ...` | Automatic (function lifetime) | On function entry | On function exit |
...
```cursive
function example() {
    let func_var = 42  // Automatic storage, lives until end of function
...
1. Data must outlive the function that creates it
...
// ========== Function Demonstrating All Storage Durations ==========

function demonstrate_storage() {
    // Automatic storage (function scope)
    let func_var = Resource::new("func_var")
...
// Destruction order (LIFO): func_var destroyed

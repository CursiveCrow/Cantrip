...
**Section**: §9.1 Statement Fundamentals  
**Stable label**: [stmt.fundamental]  
**Forward references**: §9.2 [stmt.simple], §9.3 [stmt.control], §9.4 [stmt.order], Clause 5 §5.7 [decl.initialization], Clause 8 [expr], Clause 11 [memory], Clause 12 [contract]
...
[1] A _statement_ is a syntactic form that executes for its effects on program state and control flow. Unlike expressions (Clause 8), statements do not directly produce values for consumption; instead, they modify bindings, transfer control, or orchestrate execution order.

[2] This clause specifies statement syntax, execution semantics, and control-flow behavior. Statements participate in the grant system (Clause 12) by sequencing grant-requiring operations and in the memory model (Clause 11) through binding initialization and cleanup.

[3] The execution model uses judgment $\langle s, \sigma \rangle \Downarrow \text{outcome}$, where statement $s$ transforms store $\sigma$ and produces a control outcome. Control outcomes classify normal completion, early returns, loop exits, and abnormal termination.

#### §9.1.2 Statement vs Expression Distinction

[4] Cursive distinguishes statements from expressions:

- **Expressions** (Clause 8) produce typed values and use judgment $\Gamma \vdash e : \tau \; ! \varepsilon$.
- **Statements** (this clause) execute for effects and use judgment $\langle s, \sigma \rangle \Downarrow \text{outcome}$.

[5] Control-flow constructs (`if`, `match`, `loop`) are defined as expressions in Clause 8. When used in statement position, their result values are discarded. This clause specifies how statements sequence, how control outcomes propagate, and how statement-specific forms (assignments, `return`, `defer`) integrate with the expression-based control flow.
...
[19] Statements integrate with Clause 8 expressions in three ways:

(19.1) **Expression statements**: Any expression may be used as a statement; its value is discarded (§9.2.3).

(19.2) **Control-flow expressions**: Constructs like `if`, `match`, `loop` (Clause 8 §8.4) may be used as statements. Their execution follows Clause 8 semantics; when used as statements, result values are discarded.

(19.3) **Subexpressions**: Statements contain expressions (initializers, conditions, operands). Expression evaluation follows Clause 8 rules; statement execution sequences those evaluations.
...
#### §9.1.11 Conformance Requirements

[22] Implementations shall:

1. Support all statement forms specified in §§9.2–9.3
2. Implement the execution judgment with control outcome propagation per §9.1.7
3. Enforce well-formedness checks and emit diagnostics for ill-formed statements
4. Maintain statement termination rules from §2.4
5. Integrate statements with expression evaluation, definite assignment, and grant checking
6. Execute defer blocks in LIFO order on all control-flow exits
7. Provide structured control flow only (no goto)

---

**Previous**: Clause 8 — Expressions (§8.8 [expr.typing]) | **Next**: §9.2 Simple Statements (§9.2 [stmt.simple])

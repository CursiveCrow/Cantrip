...
[2] This subclause provides the normative grammar; semantics for scope formation, visibility, and diagnostics appear in §§4.3–4.5.
...
[1] The `module_path` in any `import` or `use` shall resolve under §4.1.3; missing modules trigger diagnostics defined in §4.6.

[2] `public use` marks the clause as re-exporting the referenced items; its semantics are covered in §4.4.
...
[4] Within a `use_list`, each specifier shall be unique; duplicates provoke diagnostic E04-100.

[5] Wildcard clauses (`:: *`) shall be expanded during scope formation (§4.3); conflicts at expansion time emit diagnostic E04-101.

[6] `qualified_path :: identifier` forms are resolved according to §4.5; ill-formed targets (missing or non-public items) are diagnosed in §4.4. A `qualified_path` whose head component is an import alias refers to the aliased module.

[7] `visibility_modifier` shall not appear on `import` declarations; attempts to do so provoke diagnostic E04-102.
...
```cursive
use math::geometry::{area_of_circle, area_of_circle}  // error[E04-100]
```
...
```cursive
public import math::geometry  // error[E04-102]
```

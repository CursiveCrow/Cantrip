...
**Section**: §12.8 Verification Modes and Dynamic Hooks to Witnesses
...
### §12.8 Verification Modes and Dynamic Hooks to Witnesses [contract.verification]
...
[3] This subclause specifies verification mode syntax, semantics, checking strategies, and integration with the witness system (Clause 13) for dynamic verification hooks.

#### §12.8.2 Verification Mode Specification [contract.verification.modes]

##### §12.8.2.1 Attribute Syntax
...
##### §12.8.2.2 Mode Semantics

**static mode**:
[6] Contracts must be proven at compile time. If static verification fails, compilation fails with diagnostic E12-056. No runtime checks are inserted.
...
#### §12.8.3 Verification Mode Selection [contract.verification.selection]
...
#### §12.8.4 Static Verification Strategy [contract.verification.static]
    ...
    [[ |- n >= 0, n <= 12 => result >= 1 ]]
{
    if n <= 1 {
        ...
        // If provable: ✅
        // If not provable: ❌ E12-056
    }
}
```

#### §12.8.5 Dynamic Verification Strategy [contract.verification.dynamic]
...
#### §12.8.6 Trusted Mode [contract.verification.trusted]
...
#### §12.8.7 Build-Mode Defaults [contract.verification.buildmode]
...
| Verification | `--verify=static` | Yes (required)         | Error E12-056        | No checks (proven) |

[22] The default mode when no flags are provided is Debug (conservative, safe).

#### §12.8.8 Verification Mode and Type Signatures [contract.verification.types]
...
#### §12.8.9 Dynamic Hooks to Witness System [contract.verification.witness]

[24] When dynamic verification is enabled, the compiler may generate _witness hooks_: runtime evidence that contracts are satisfied. Witnesses integrate with the witness system (Clause 13) for dynamic polymorphism.
...
[26] The witness system specification (Clause 13) defines how runtime evidence is represented, stored, and used for dynamic dispatch. The contract system provides the verification points where witnesses are generated.

[ Note: Complete witness integration semantics are specified in Clause 13 [witness]. This clause establishes that dynamic verification generates witness evidence; Clause 13 specifies how that evidence is used.
— end note ]

#### §12.8.10 Constraints [contract.verification.constraints]

[1] _Attribute placement._ The `[[verify(...)]]` attribute shall appear directly before the procedure declaration it modifies. Placing it elsewhere produces diagnostic E12-080.

[2] _Mode validity._ The verification mode shall be one of: `static`, `dynamic`, `trusted`. Invalid modes produce diagnostic E12-081.

[3] _Static verification requirement._ When mode is `static`, all contracts in the procedure must be provable at compile time. Unprovable contracts in static mode produce diagnostic E12-056.
...
[5] _Build flag validity._ The compiler flags `--build=debug`, `--build=release`, and `--verify=none` are implementation-defined but shall follow the semantics in Table 12.3.

#### §12.8.11 Examples [contract.verification.examples]
...
#### §12.8.12 Diagnostics [contract.verification.diagnostics]

[6] Verification mode diagnostics:

| Code    | Condition                                              | Constraint |
| ------- | ------------------------------------------------------ | ---------- |
| E12-056 | Static verification failed (mode=static)               | [3]        |
| E12-080 | verify attribute misplaced (not before procedure)      | [1]        |
| E12-081 | Invalid verification mode (not static/dynamic/trusted) | [2]        |

#### §12.8.13 Conformance Requirements [contract.verification.requirements]

[7] Implementations shall:

1. Support three explicit verification modes: static, dynamic, trusted
2. Provide build-mode-dependent defaults per Table 12.3
...
10. Emit diagnostics E12-056, E12-080, E12-081 for mode violations
11. Document build flags and their verification behavior

---

**Previous**: §12.7 Sequent Checking Flow (§12.7 [contract.checking]) | **Next**: §12.9 Grammar Reference (§12.9 [contract.grammar])

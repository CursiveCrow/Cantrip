...
#### §8.4.4 Conditional Expressions (`if` and `if let`)
...
[19] Exhaustiveness: matches over enums, unions, modal states, and booleans must cover all constructors. Non-exhaustive matches emit E08-451 with a list of missing cases. Catch-all `_` arms satisfy the requirement.
...
#### §8.4.7 Closure Expressions and Capture [expr.structured.closure]
...
##### §8.4.7.3 Free Variable Identification [expr.structured.closure.free]
...
##### §8.4.7.4 Capture Modes [expr.structured.closure.modes]
...
##### §8.4.7.5 Environment Record Synthesis [expr.structured.closure.environment]
...
#### §8.4.8 Scope interaction and temporaries
